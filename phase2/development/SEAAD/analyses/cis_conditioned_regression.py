...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Run conditioned regression analysis (endo ~ target_variable + exog + covariates)."
    )
...
def process_cell_type(
    ...
    def resolve_covariates(arg_covar, arg_covar_list, original_cols):
        if arg_covar == "none":
            ...
            raw_covars = [c for c in original_cols if c != args.target_variable]
        elif arg_covar == "knowns":
            raw_covars = [c for c in original_cols if not ("PCA_" in c) and c != args.target_variable]
        elif arg_covar == "unknowns":
            raw_covars = [c for c in original_cols if ("PCA_" in c)]
        elif arg_covar == "specified":
            raw_covars = [c for c in arg_covar_list if c != args.target_variable]
        else:
            raw_covars = []
    ...
    all_covar_cols = list(
        dict.fromkeys([args.target_variable, endo_weight_term] + endo_formula_covariates)
    )
...
def main():
    ...
    logger.info(f"Loaded {len(sig_results)} significant target-variable-associated cis pairs.")
