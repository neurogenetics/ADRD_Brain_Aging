...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Build gene metrics from UCSC knownGene CSV."
    )
    parser.add_argument(
        "--input",
        type=str,
...
def main():
    ...
    with open(input_file, "r", encoding="utf-8") as f:
        ...
        for i, line in enumerate(f):
            ...
            try:
                ...
                exonEnds = [
                    int(x) for x in parts[starts_end_idx : ends_end_idx + 1] if x
                ]
                ...
                tx_gene_size = sum(
                    end - start for start, end in zip(exonStarts, exonEnds)
                )
    ...
    transcript_counts = (
        df.groupby("geneSymbol")["name"].nunique().rename("num_transcripts")
    )
    ...
    max_gene_size = (
        df.groupby("geneSymbol")["tx_gene_size"].max().rename("max_gene_size")
    )

    # 4. Genomic region size: max 'tx_genomic_region_size'
    max_genomic_size = (
        df.groupby("geneSymbol")["tx_genomic_region_size"]
        .max()
        .rename("max_genomic_region_size")
    )

    # 5. Chromosome: first 'chrom'
    chromosome = df.groupby("geneSymbol")["chrom"].first().rename("chromosome")

    # Combine into a single DataFrame
    metrics_df = pd.concat(
        [transcript_counts, max_exons, max_gene_size, max_genomic_size, chromosome],
        axis=1,
    )
