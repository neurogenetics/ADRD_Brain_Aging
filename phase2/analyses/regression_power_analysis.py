...
def compute_empirical_alpha(file_path):
    logger.info(f"Reading WLS regression results from: {file_path} to compute alpha...")
    try:
    ...
    logger.info(f"  -> Computed empirical alpha: {alpha:.4e}")
    return alpha
...
def plot_variance_distribution(df, label, n, var_threshold, target_power, output_dir):
    ...
    # Plot non-significant points (backbone of the curve)
    ax.scatter(
        x_nonsig,
        y_nonsig,
        color="#3b6a94",
    ...
    # Highlight significant features (smaller dots on top)
    if len(x_sig) > 0:
        ax.scatter(
            x_sig,
            y_sig,
    ...
    plt.close(fig)
    logger.info(f"Visualization successfully saved to: {dist_fig_path}")
    logger.info("=" * 65 + "\n")
...
def process_dataset(
    ...
    logger.info(f"Empirical Alpha: {alpha:.4e}")
    logger.info(
    ...
    # 1. Load results file
    logger.info(f"Reading WLS regression results from: {results_path}...")
    try:
        df = pd.read_csv(
            results_path,
            sep=",",
    ...
    logger.info("\n" + "-" * 50)
    logger.info(f"Empirical Threshold from {int(target_power * 100)}% Power:")
    logger.info(f"  -> R-Squared: {var_threshold:.2f}%")
    logger.info(f"  -> Approx Percent Change: {empirical_pct_change_at_thresh:.2f}%")
    logger.info(f"Total Significant Features (fdr_bh <= 0.05): {total_sig:,}")
    logger.info(
        f"Significant Features >= Threshold: {count_above:,} ({pct_above:.2f}%)"
    )
    ...
    if "tissue" in sig_df.columns:
        ...
        for tissue, total in tissue_counts.items():
            ...
            logger.info(f"  -> {tissue}: {above:,} / {total:,} ({pct:.2f}%)")
    else:
