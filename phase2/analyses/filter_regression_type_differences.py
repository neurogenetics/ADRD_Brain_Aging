...
def main():
    ...
    general_fdr_file = (
        results_dir / f"{project}.all_celltypes.{modality}.{general_type}_fdr.{target_variable}.csv"
    )
    # Robust input: Full results (to be filtered by nominal p < 0.05)
    robust_file = (
        results_dir / f"{project}.all_celltypes.{modality}.{robust_type}.{target_variable}.csv"
    )
    ...
    try:
        ...
        plt.title(f"Distribution of Effect Delta Z-scores ({modality} - {target_variable.upper()})")
        plt.xlabel(f"Z-score of (Coef {general_type.upper()} - Coef {robust_type.upper()})")
        fig_path = figures_dir / f"{project}.{modality}.effect_delta_z_distribution.{target_variable}.png"
        plt.savefig(fig_path)
        plt.close()
        logger.info(f"Saved Z-score distribution plot to {fig_path}")
    except Exception as e:
        logger.warning(f"Failed to generate Z-score distribution plot: {str(e)}")
