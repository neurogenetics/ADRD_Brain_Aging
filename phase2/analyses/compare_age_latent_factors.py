...
def parse_args():
    ...
    parser.add_argument(
        "--fdr-threshold", type=float, default=0.05, help="FDR threshold for inclusion (significant factors)."
    )
    parser.add_argument(
        "--corr-method", type=str, choices=["spearman", "pearson"], default="spearman", help="Correlation method to use."
    )
    return parser.parse_args()


def load_sig_factors(results_dir, project, modality, fdr_threshold, per_cell):
    regression_type = "lmm" if per_cell else "pb_wls"
    fdr_file = results_dir / "latents" / "cnmf" / ("lmm_results" if per_cell else "wls_pb_results") / f"{project}_combined_{modality}_{regression_type}_fdr.csv"
    ...
    if "fdr_age" not in df.columns:
        logger.warning(f"'fdr_age' column missing in {fdr_file}")
        return pd.DataFrame()
        
    sig_df = df[df["fdr_age"] <= fdr_threshold].copy()
    sig_df["modality"] = modality
    logger.info(f"Loaded {len(sig_df)} significant factors (fdr_age <= {fdr_threshold}) for {modality}.")
    return sig_df
...
def main():
    ...
    rna_sig = load_sig_factors(results_dir, args.project, "rna", args.fdr_threshold, args.per_cell)
    atac_sig = load_sig_factors(results_dir, args.project, "atac", args.fdr_threshold, args.per_cell)
    ...
    try:
        ...
        obs_df["safe_cell_label"] = obs_df["cell_label"].str.replace(" ", "_").str.replace("/", "-")
    ...
    for (ct, mod, k), group in grouped_factors:
        ...
        try:
            usage, *_ = cnmf_obj.load_results(K=k, density_threshold=args.density_threshold)
        except Exception as e:
            logger.error(f"Failed to load cNMF results for {run_name} K={k}. Error: {e}")
            continue

        modality_list = MODAL_TYPES_DICT.get(mod, [mod])
        ct_obs = obs_df[(obs_df.modality.isin(modality_list)) & (obs_df["safe_cell_label"] == ct)].copy()
        ...
        if len(usage) == 0:
            logger.warning(f"No cells left after intersecting cNMF usages with obs for {run_name}")
            continue

        # Add sample_id for pseudobulking
        usage_with_sample = usage.merge(ct_obs[["sample_id"]], left_index=True, right_index=True, how="left")
        ...
        if not args.per_cell:
            ...
            logger.warning("Comparing across cell types requires a common index (like sample_id). Forcing pseudobulk for cross-comparison.")
            usage_with_sample = usage_with_sample.dropna(subset=["sample_id"])
            agg_usage = usage_with_sample.groupby("sample_id").mean()
        ...
        for factor_idx in group["factor"].values:
            ...
            if factor_str in agg_usage.columns:
                ...
                if int(factor_idx) in agg_usage.columns:
                    ...
                    logger.warning(f"Factor {factor_idx} not found in usage columns for {run_name} K={k}")
    ...
    logger.info(f"Combined usage matrix shape: {combined_usages.shape} (Samples x Factors)")
    ...
    for i in range(n_cols):
        for j in range(i + 1, n_cols):
            valid_mask = combined_usages.iloc[:, i].notna() & combined_usages.iloc[:, j].notna()
            mask_arr = valid_mask.values
            if mask_arr.sum() >= 3:
                _, p = corr_func(combined_usages.iloc[mask_arr, i], combined_usages.iloc[mask_arr, j])
    ...
    corr_out = results_dir / "latents" / "cnmf" / f"{args.project}_age_factors_correlation_{file_suffix}.csv"
    corr_matrix.to_csv(corr_out)
    
    # Save FDR matrix
    fdr_out = results_dir / "latents" / "cnmf" / f"{args.project}_age_factors_fdr_{file_suffix}.csv"
    ...
    modality_colors = {"RNA": "tab:blue", "ATAC": "tab:orange"}
    ...
    try:
        ...
        g.ax_heatmap.set_yticklabels(g.ax_heatmap.get_ymajorticklabels(), fontsize=8, rotation=0)
        
        heatmap_out_png = figs_dir / f"{args.project}_age_factors_clustermap_{file_suffix}.png"
        heatmap_out_svg = figs_dir / f"{args.project}_age_factors_clustermap_{file_suffix}.svg"
