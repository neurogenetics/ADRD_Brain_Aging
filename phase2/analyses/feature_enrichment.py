...
def parse_args():
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="Calculate enrichment based on cell-type specificity (derived from the WLS results).",
    )
    parser.add_argument(
        "--permutations",
        type=int,
    ...
    for col in cat_cols:
        ...
        for i in range(n_perms):
            sampled_features = np.random.choice(
                feature_indices, size=n_sig, replace=False
            )
...
def run_overlap_permutations(
    ...
    if not all(col in bg_bed_df.columns for col in coord_cols):
        logger.warning(
            f"Background features missing coordinate columns {coord_cols}. Found: {bg_bed_df.columns.tolist()}"
        )
        return None
    ...
    logger.info(
        f"Total overlapping features in background: {len(overlapped_feature_ids)} / {len(background_df)}"
    )
...
def main():
    ...
    log_filename = (
        logs_dir / f"{args.project}.{args.modality}{name_suffix}.feature_enrichment.log"
    )
    ...
    features_df = pd.read_csv(
        features_file, index_col=0
    )  # Assuming first column is the feature ID
    ...
    if args.metrics_file:
        ...
        logger.info(
            f"Found {len(metrics_cols_num)} numeric metrics: {metrics_cols_num}"
        )
        logger.info(
    ...
    if not metrics_cols_num and not metrics_cols_cat and annotation_bed is None:
        logger.warning(
            "No metrics, annotations, or specificity flags provided. Nothing to test."
        )
        sys.exit(0)
    ...
    for cell_type, ct_group in wls_df.groupby("tissue"):
        ...
        if len(sig_features) == 0:
            logger.warning(
                f"  No significant features found for {cell_type}. Skipping."
            )
            continue
        ...
        if len(valid_bg) == 0:
            logger.warning(
                "  None of the tested features exist in the features file. Skipping."
            )
            continue
    ...
    if all_results:
        ...
        out_file = (
            results_dir
            / f"{args.project}.{args.modality}{name_suffix}.feature_enrichment.csv"
        )
