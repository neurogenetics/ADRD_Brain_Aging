...
6. Feature Enrichment and Permutation Testing (feature_enrichment.py)
   ...
   * Data Integration:
       ...
       * Subsets this background into a "significant" pool based on a user-defined False Discovery Rate (FDR) threshold (default: < 0.05).
       * Merges feature coordinates and optionally integrates pre-computed structural metrics (e.g., transcript counts, exon counts, gene size) and cell-type specificity metrics derived directly from the regression results.
   * Statistical Methodology (Permutation Testing):
       * To robustly handle non-normally distributed metrics and varying pool sizes, the analysis employs non-parametric resampling.
       * During each iteration, a set of features—equal in size to the number of significant features—is randomly sampled without replacement from the cell-type-specific background pool.
       * Two-tailed empirical p-values are calculated by comparing the observed metric (mean, proportion, distance, or overlap rate) in the significant pool against the null distribution generated by 1,000 (default) random permutations.
   * Testing Modalities:
       * Metric Permutations: Evaluates continuous numeric metrics (calculating fold-change of means) and categorical metrics (calculating fold-change of proportions).
       * Overlap Permutations: Utilizes `pybedtools` to calculate the proportion of features that physically overlap with a provided genomic annotation (e.g., Human Accelerated Regions [HARs] or Human Ancestor Quickly Evolved Regions [HAQERs]). Overlaps are pre-calculated on the full background to optimize iteration speed.
