...
def main():
    ...
    # Configure logging
    log_filename = (
        f"{args.modality}_{args.cell_type}_{args.feature}_specific_analysis.log"
    )
    logging.basicConfig(
        level=logging.INFO,
    ...
    modality = (
        args.modality.lower()
    )  # ensure lowercase to match file patterns in scripts
    ...
    if all_results_file.exists():
        ...
        specific_result = glm_results.loc[
            (glm_results.feature == feature) & (glm_results.tissue == cell_type)
        ]
        if not specific_result.empty:
            ...
            logger.warning(
                f"No result found for {feature} in {cell_type} in aggregated file."
            )
    else:
    ...
    pb_file = quants_dir / f"{project}.{cell_type}.{modality}.residuals.parquet"
    ...
    covars_file = (
        work_dir
        / "sample_info"
        / f"{project}.{cell_type}.{modality}.final_covariates.csv"
    )
    ...
    data_df = quants_df[[feature]].merge(
        covars_df, how="inner", left_index=True, right_index=True
    )
    ...
    plt.savefig(
        figures_dir / f"{project}.{modality}.{cell_type}.{feature}.lm_robust.png"
    )
    plt.close()
